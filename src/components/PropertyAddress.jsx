...
export default function PropertyAddress({ onSave }) {
  ...
  return (
    <form>
      <div className="grid sm:grid-cols-3 grid-cols-1 gap-[16px]">
        <Input label="Property name as identifier" required placeholder="Enter apartment name" value={formValues.propertyName} onChange={handleChange("propertyName")} />
        <Input label="Total apartment unit" type="number" required placeholder="Enter apartment unit" value={formValues.unitCount} onChange={handleChange("unitCount")} />
        <Input label="Property website" optional type="url" placeholder="Enter Property website" value={formValues.website} onChange={handleChange("website")} />

        <Dropdown label="Country/Region" required options={countries} value={formValues.country} onChange={handleChange("country")} />
        <Input label="Street address" required placeholder="Enter street address" value={formValues.street} onChange={handleChange("street")} />
        <Input label="Apt, suit, unit" type="number" placeholder="Enter Apt/suit/unit" value={formValues.apt} onChange={handleChange("apt")} />
        <Input label="City/Town" required placeholder="Enter city/town" value={formValues.city} onChange={handleChange("city")} />
        <Dropdown label="State/Territory" required options={countries} value={formValues.state} onChange={handleChange("state")} />
        <Input label="Zip code" required placeholder="Enter zip code" value={formValues.zip} onChange={handleChange("zip")} />
      </div>
